...
function onBot() {
  ...
  login(loginData, async (loginError, loginApiData) => {
      ...
      (async () => {
        ...
        for (const command of listCommand) {
          try {
            ...
            try {
              global.loading(`${cra(`LOADED`)} ${cb(config.name)} success`, "𝗡𝗔𝗭𝗥𝗨𝗟 𝗖𝗠𝗗");
            } catch (err) {
    ...
    global.loading(`${cra(` TIME START `)} Launch time: ${((Date.now() - global.client.timeStart) / 1000).toFixed()}s`, "LOADED");
...
/* *
This bot was created by me (NAZRUL) and my brother Frogs. Do not steal my code. (つ ͡ ° ͜ʖ ͡° )つ ✄ ╰⋃╯
