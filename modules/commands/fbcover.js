...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  switch (handleReply.type) {
    case "tenphu": {
      ...
      return api.sendMessage(`♻️আপনার সহজ fb এর কিছু নাম দিয়া হলো:
      red,blue,orange,green, yellow,gray,lime: ${event.body.toUpperCase()}\n\n(এই বার্তার উত্তরে আপনার ফোন নম্বর লিখুন)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "sdt",
          name: `fbcover`,
          author: senderID,
    ...
    case "sdt": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`♻️আপনি এইভাবে SDT নির্বাচন করেছেন: ${event.body.toUpperCase()}\n\n
(আপনার ইমেল লিখতে এই বার্তার উত্তর দিন)`,threadID, function (err, info) {
    ...
    case "email": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`♻️ আপনি এই হিসাবে ইমেল নির্বাচন করেছেন: ${event.body.toUpperCase()}\n\n(আপনার ঠিকানা লিখতে এই বার্তার উত্তর দিন)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "color",
          name: `fbcover`,
          author: senderID,
    ...
    case "color": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`♻️আপনি এই হিসাবে ঠিকানা নির্বাচন করেছেন: ${event.body.toUpperCase()}\n\nআপনার কভারের ব্যাকগ্রাউন্ডের রং বাছাই করতে এই মেসেজ এর রিপ্লাই দিন (enter no as default color)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "create",
          name: `fbcover`,
          author: senderID,
    ...
    case "create": {
      ...
      api.unsendMessage(handleReply.messageID);
      api.sendMessage(`🍁অনুগ্রহ করে অপেক্ষা করুন আপনার কভার তৈরি হচ্ছে....!`,threadID, (err, info) => {
      setTimeout(() => {
