...
const validCommands = ['cartoon', 'photo', 'lofi', 'sad', 'islamic','funny','horny','anime','love','baby','lyrics','sigma','photo','aesthetic','cat'];
  ...
  if (args[0] === 'list'){
 try {
   const lRes = await axios.get(`https://d1pt0-album.onrender.com/data?list=dipto`);
...
api.sendMessage(`${error}`,event.threadID,event.messageID)
  ...
  query2 = query;} else {
        ...
        const svRes = await axios.get(`https://d1p-imgur.onrender.com/data?${query2}=${imgurLink}`);
...
module.exports.handleReply = async function ({ api, event, handleReply }) {
  ...
  //console.log(query);
  try {
    const res = await axios.get(`https://d1pt0-album.onrender.com/data?type=${query}`);
